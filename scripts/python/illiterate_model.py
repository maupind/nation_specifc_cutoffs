...
# Loading dataset, import path to the made dataset from "document name"
clin0 = feather.read_feather('/path/to/your/data/dir/lasi.feather')

# Filter participants to those who are illiterate
clin0['r1illiterate'] = clin0['r1illiterate'].astype(int)
clin0 = clin0[clin0['r1illiterate'] == 1]

### REMOVE RURAL AND ILLITERATE VARIABLES FROM NEXT RUN
...
# Changing data types for the outcome variable
le = LabelEncoder()
y = le.fit_transform(y)

# Define columns whose data types will be changed to factors
...
# Instantiate lists to collect SHAP values and AUC scores for each fold
all_shap_values = []
all_val_data = []
all_thresholds = []
best_model = None
...
X_transform = X_transform.rename(
    columns = {
        ...
        'r1iqscore14' : 'IQCODE - Handling Financial Matter',
...
# Define the search space of hyperparameter optimisation. Values provide boundaries for to search for optimal parameters within
search_spaces = {
...
# Split the data into 70% training and 30% testing, stratified by outcome
...
# Define the function that will be used to implement Bayesian Optimisation
...
for train_idx, val_idx in skf.split(X_train_full, y_train_full):
    ...
    for i in range(num_iterations):
        ...
        print(torch.isnan(X_unnorm).sum())

        # Compute objective
        objective_value = objective_function(X_unnorm)
        print(np.isnan(objective_value).sum())
    ...
    # Define the expected improvement equation
    ei = ExpectedImprovement(gp, best_f=fold_best_auc)

    # Save outcomes if the calculated fold AUC is better than the previous best
...
# Predict the probabilites on the test dataset
...
#joblib.dump(best_model, "/path/to/your/wd/model_name")

# Save the SHAP values and associated data for future use
#joblib.dump(all_shap_values_stacked, '/path/to/your/wd/file_name')
#joblib.dump(all_val_data_stacked, '/path/to/your/wd/file_name')
# Average SHAP values across all folds and plot summary
#avg_shap_values = np.mean(all_shap_values, axis=0) 

# Initialise plotting for the SHAP values
shap.initjs()
# Create the SHAP summary plots
shap.summary_plot(all_shap_values_stacked, all_val_data_stacked)
